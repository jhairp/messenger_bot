...
def send_buttons(recipient_id, text, botones):
    """Envia tarjeta con botones rectangulares (maximo 3 botones)."""
    buttons = [
        {"type": "postback", "title": btn["title"], "payload": btn["payload"]}
        for btn in botones[:3]  # Facebook permite maximo 3
    ]
    send_request({
        "messaging_type": "RESPONSE",
        "recipient": {"id": recipient_id},
...
def menu_principal(sender_id):
    send_buttons(sender_id,
        "Hola! Bienvenido a Streaming Free.\nQue deseas hacer?",
        [
...
def menu_catalogo(sender_id):
    send_buttons(sender_id,
        "Tenemos estas plataformas. Elige una:",
        [
...
def info_plataforma(sender_id, plataforma):
    datos = {
        "INFO_NETFLIX":     ("Netflix compartido\n1 pantalla: $3/mes\n2 pantallas: $5/mes\nAcceso inmediato, sin cortes.",   "Comprar Netflix",     "PEDIR_NETFLIX"),
...
def iniciar_compra(sender_id, plataforma):
    ...
    send_text(sender_id, f"Excelente eleccion! Para completar tu compra de {nombre}, un asesor te contactara con los datos de pago en breve.")
    send_buttons(sender_id, "Puedo ayudarte con algo mas?", [
...
def handle_message(sender_id, message):
    if "text" in message:
        ...
        if any(s in texto for s in saludos):
            ...
            send_buttons(sender_id, "Usa el menu para navegar:", [
                {"title": "Ver catalogo", "payload": "CATALOGO"},
                {"title": "Precios",      "payload": "PRECIOS"},
                {"title": "Menu principal","payload": "MENU"},
            ])
